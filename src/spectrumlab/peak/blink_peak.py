...
def find_maxima(values: Sequence[R]) -> tuple[Number]:
    ...
    for i in range(1, n_values-1):
        conditions = (
            values[i-1] < values[i] and values[i] >= values[i+1],
            # values[i-1] <= values[i] and values[i] > values[i+1],  # NOT USE IT: в случае, если подряд два отсчета с одинаковыми значениями, то максимум считается дважды!  # noqa: E501
        )
        if any(conditions):
            extrema.append(i)
...
def draft_blinks(
    spectrum: Spectrum,
    noise: Noise,
    config: DraftBlinkPeakConfig | None = None,
    show: bool = False,
) -> tuple[BlinkPeak]:
    ...
    for maximum, pair in zip(maxima, pairs):
        ...
        _amplitude = spectrum.intensity[maximum] - (spectrum.intensity[left] + spectrum.intensity[right])/2  # от среднего значения на границах до максимума  # noqa: 501
