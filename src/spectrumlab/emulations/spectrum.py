        ...
        **kwargs,
        ...
        match method:
            ...
            case 'weighted':
                ...
                intensity = np.array([
                    np.dot(intensity, w)
                    for intensity, w in zip(intensity.T, weight.T)
                ] / np.sum(weight, axis=0))
                deviation = np.sqrt(np.array([
                    np.dot(deviation[deviation < np.infty]**2, w[deviation < np.infty]**2)
                    for deviation, w in zip(deviation.T, weight.T)
                ] / np.sum(weight, axis=0) ** 2))
                clipped = np.min([spe.clipped for tau, spe in shorts.items()], axis=0)
