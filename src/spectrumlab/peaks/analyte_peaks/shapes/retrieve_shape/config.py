...
DEFAULT_SHAPE = PeakShape(width=2, asymmetry=0, ratio=.1)


class RetrieveShapeConfig(BaseSettings):

    default_shape: PeakShape = Field(None, alias='RETRIEVE_SHAPE_DEFAULT')
    ...
    @classmethod
    def validate_default_shape(cls, data: str | None) -> PeakShape:
        ...
        try:
            ...
            shape = PeakShape(width=width, asymmetry=asymmetry, ratio=ratio)
        except Exception:
            shape = DEFAULT_SHAPE
        return shape
